...
* -----------------------------------generate_random_hierarchical_graphlet.cpp --------------------------------------
*
...
#define	LIST_HIERARCHICAL_EDGES_IN prhs[1]
#define NUM_GRAPHLETS_IN prhs[2]
#define MAX_SIZE_IN prhs[3]
// Output Arguments
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	if(nrhs<4) 
	{
		mexErrMsgTxt("\nError: Input arguments must be four.\n");
		return;
	}
	if(nlhs>1)
	{
    ...
    int hierarchical_edges = 0;
    if(mxGetM(LIST_HIERARCHICAL_EDGES_IN)>0)
    {
        hierarchical_edges = 1;
        if (mxGetN(LIST_HIERARCHICAL_EDGES_IN)!=2)
        {
            char buffer [150];
            sprintf (buffer, "\nError: Check the dimension N of list of hierarchical edges.\nRecieved %lu instead of 2.\n", mxGetN(LIST_HIERARCHICAL_EDGES_IN));
            mexErrMsgTxt(buffer);
            return;
        }
    }
